    ...
    <form class="theme-form login-form">
      ...
      <div class="form-builder-header-1 mb-4">
        <ul class="nav nav-primary justify-content-center">
          <li class="nav-item" @click="codeToggle"><a class="nav-link m-r-5" :class="{ active: code == '100' }" style="border-radius: 5px">일반회원</a></li>
          <li class="nav-item" @click="codeToggle"><a class="nav-link m-r-5" :class="{ active: code == '200' }" style="border-radius: 5px">기업회원</a></li>
        </ul>
      </div>
      ...
      <div class="form-group">
        ...
        <div class="small-group">
          <div class="input-group">
            ...
            <input class="form-control" :class="{ 'input-error': isUserName }" type="text" placeholder="Name" v-model="userName" />
          </div>
        </div>
      </div>
      ...
      <div class="form-group">
        ...
        <div class="input-group">
          ...
          <input class="form-control" :class="{ 'input-error': isUserEmail }" type="email" placeholder="your-email@domain.com" @change="emailCheck" v-model="userEmail" />
        </div>
        <div
          v-show="popupEmail"
          class="popover fade show bs-popover-end"
      ...
      <div class="form-group">
        ...
        <div class="input-group">
          ...
          <input class="form-control" :class="{ 'input-error': isUserPassword }" type="password" placeholder="*********" v-model="userPassword" />
        </div>
      </div>
      <div class="form-group">
        ...
        <div class="input-group">
          <span class="input-group-text"><i class="icon-lock"></i></span>
          <input class="form-control" :class="{ 'input-error': isUserPasswordCheck }" @change="passwordCheck" type="password" placeholder="*********" v-model="userPasswordCheck" />
        </div>

        <div
          v-show="popupPassword"
          class="popover fade show bs-popover-end"
          style="margin: 0px; position: absolute; inset: 0px auto auto 0px; transform: translate(400px, 10px)"
          data-popper-placement="right"
        >
      ...
      <div class="form-group">
        ...
        <div class="input-group">
          ...
          <input class="form-control" :class="{ 'input-error': isUserTel }" type="text" placeholder="010-1234-5678" v-model="userTel" />
        </div>
      </div>
      ...
      <div class="form-group">
        <div class="row">
          <div class="col-xl-4 col-sm-12 col-md-12 mb-2">
            ...
            <select class="form-select form-control" :class="{ 'input-error': isSelectedSido }" v-model="selectedSido" @change="getGugunList">
              <option v-for="(sido, index) in sidoList" :key="`sido-${index}`" :value="sido">{{ sido.name }}</option>
            </select>
          </div>
          <div class="col-xl-4 col-sm-12 col-md-12 mb-2">
            <label class="form-label">구 / 군</label>
            <select class="form-select form-control" :class="{ 'input-error': isSelectedGugun }" v-model="selectedGugun" @change="getDongList">
              <option v-for="(gugun, index) in gugunList" :key="`gugun-${index}`" :value="gugun">{{ gugun.name }}</option>
            </select>
          </div>
          <div class="col-xl-4 col-sm-12 col-md-12 mb-2">
            ...
            <select class="form-select form-control" :class="{ 'input-error': isSelectedDong }" v-model="selectedDong">
              <option v-for="(dong, index) in dongList" :key="`dong-${index}`" :value="dong">{{ dong.name }}</option>
            </select>
          </div>
        </div>
        <div class="row">
          <div class="col-xl-12 col-sm-12 col-md-12 mb-2">
            ...
            <input class="form-control" :class="{ 'input-error': isDetailAddress }" type="text" placeholder="상세 주소" v-model="detailAddress" />
          </div>
        </div>
      </div>
    ...
    async register() {
      if (this.inputCheck()) {
        const user = {
          ...
          userAddress: `${this.selectedSido} ${this.selectedGugun} ${this.selectedDong}`,
